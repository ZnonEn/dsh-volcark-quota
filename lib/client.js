	...
	factory: (require) => {
		...
		*/
			...
			const load = react.default.useCallback(async () => {
				...
				try {
					const json = await (await fetch(API_URL, {
						...
						body: JSON.stringify({
							ak: localStorage.getItem(LS_AK) || void 0,
							sk: localStorage.getItem(LS_SK) || void 0,
							planType: localStorage.getItem(LS_PLAN) || "auto"
						})
					})).json();
		...
		function ConfigCard() {
			...
			const [ak, setAk] = react.default.useState(() => localStorage.getItem(LS_AK) || "");
			const [sk, setSk] = react.default.useState(() => localStorage.getItem(LS_SK) || "");
			...
			return h("li", { className: "vq-card" }, h("button", {
				...
				className: "vq-card-label",
				htmlFor: "vq-cfg-sk"
			}, "Secret AccessKey"), h("input", {
				id: "vq-cfg-sk",
				className: "vq-card-input",
				type: "password",
				value: sk,
				placeholder: "仅写入本机 localStorage，host 不落盘",
				onChange: edit(setSk)
			})), h("div", { className: "vq-card-field" }, h("label", {
				className: "vq-card-label",
				htmlFor: "vq-cfg-plan"
			...
			}, h("option", { value: "auto" }, "自动探测（coding → agent）"), h("option", { value: "coding" }, "Coding Plan"), h("option", { value: "agent" }, "Agent Plan"))), h("div", { className: "vq-card-hint" }, "凭据仅存本机浏览器，随请求发给 127.0.0.1 的 DSH 服务；也可改用环境变量 VOLC_ACCESS_KEY_ID / VOLC_ACCESS_KEY_SECRET 启动 DSH。"), h("div", { className: "vq-card-footer" }, saved ? h("span", { className: "vq-card-saved" }, "已保存，悬浮球已刷新 ✓") : null, h("span", { className: "vq-spacer" }), h("button", {
				type: "button",
