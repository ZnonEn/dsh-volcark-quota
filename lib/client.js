window.__ModuleLoader__.load({
	id: "@dsh-external/dsh-volcark-quota",
	factory: (require) => {
		var module = { exports: {} };
		...
		*/
		...
		function panelFromBall(b) {
			const vw = typeof window !== "undefined" ? window.innerWidth : 1200;
			const vh = typeof window !== "undefined" ? window.innerHeight : 800;
			const PW = Math.min(448, vw - 24);
			const GAP = 10;
			const M = 8;
			let x = b.x + 32 > vw / 2 ? b.x - PW - GAP : b.x + 64 + GAP;
		...
		const CSS = [
			...
			".vq-card{list-style:none;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.16));border-radius:14px;background:var(--dsw-alias-bg-layer-1,#242424);overflow:hidden;box-shadow:none}",
			".vq-card-header{display:flex;align-items:center;gap:12px;width:100%;text-align:left;background:transparent;border:none;cursor:pointer;padding:18px 20px;font:inherit;color:inherit}",
			".vq-card-header:hover{background:var(--dsw-alias-bg-layer-2,rgba(255,255,255,.04))}",
			".vq-card-headtext{display:flex;flex-direction:column;gap:2px;flex:1;min-width:0}",
			".vq-card-name{font-size:17px;font-weight:700;color:var(--dsw-alias-label-primary,#f3f3f3);line-height:1.25}",
			".vq-card-desc{font-size:13px;color:var(--dsw-alias-label-tertiary,rgba(255,255,255,.62));line-height:1.45}",
			".vq-card-status,.vq-card-pending{font-size:10px;font-weight:650;border-radius:6px;padding:4px 7px;white-space:nowrap}",
			".vq-card-status{color:var(--dsw-alias-state-success-primary,#16885b);background:color-mix(in srgb,var(--dsw-alias-state-success-primary,#22c55e) 10%,transparent)}",
			".vq-card-status-pending,.vq-card-pending{color:var(--dsw-alias-state-warn-primary,#b45309);background:color-mix(in srgb,var(--dsw-alias-state-warn-primary,#f59e0b) 10%,transparent)}",
			...
			".vq-card-body{display:flex;flex-direction:column;gap:15px;padding:18px 20px 20px;border-top:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.08));background:var(--dsw-alias-bg-base,#151515)}",
			".vq-card-field{display:flex;flex-direction:column;gap:4px}",
			".vq-card-label{font-size:13px;font-weight:650;color:var(--dsw-alias-label-secondary,rgba(255,255,255,.78))}",
			".vq-card-input{box-sizing:border-box;width:100%;height:40px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.18));background:var(--dsw-alias-bg-layer-3,rgba(255,255,255,.12));border-radius:8px;padding:0 12px;font-size:13px;color:var(--dsw-alias-label-primary,#f1f1f1);font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;transition:border-color .15s ease,box-shadow .15s ease}",
			".vq-card-input:focus{outline:none;border-color:var(--dsw-alias-label-secondary,#b9b9b9);box-shadow:0 0 0 3px rgba(255,255,255,.08)}",
			".vq-card-select{box-sizing:border-box;width:100%;height:40px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.18));background:var(--dsw-alias-bg-layer-3,rgba(255,255,255,.12));border-radius:8px;padding:0 10px;font-size:13px;color:var(--dsw-alias-label-primary,#f1f1f1)}",
			".vq-card-hint{font-size:12px;color:var(--dsw-alias-label-tertiary,rgba(255,255,255,.62));line-height:1.55;padding:9px 11px;border-radius:8px;background:var(--dsw-alias-bg-layer-2,rgba(255,255,255,.08))}",
			".vq-card-footer{display:flex;align-items:center;gap:12px;padding-top:4px}",
			".vq-card-discard{min-height:40px;background:transparent;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.18));color:var(--dsw-alias-label-secondary,#d0d0d0);border-radius:8px;padding:0 14px;font-size:13px;cursor:pointer}",
			".vq-card-discard:hover{background:var(--dsw-alias-bg-layer-2,rgba(255,255,255,.08))}",
			".vq-card-save{min-height:40px;background:color-mix(in srgb,var(--dsw-alias-label-primary,#fff) 42%,var(--dsw-alias-bg-base,#161616));border:1px solid color-mix(in srgb,var(--dsw-alias-label-primary,#fff) 20%,var(--dsw-alias-bg-base,#161616));color:var(--dsw-alias-bg-base,#161616);border-radius:8px;padding:0 18px;font-size:13px;font-weight:650;cursor:pointer}",
			".vq-card-save:hover{filter:brightness(1.12)}",
			".vq-card-save:disabled{background:color-mix(in srgb,var(--dsw-alias-label-primary,#fff) 12%,var(--dsw-alias-bg-layer-2,#282828));border-color:transparent;color:var(--dsw-alias-label-tertiary,#777);opacity:1;cursor:not-allowed;filter:none}",
			".vq-card-discard:disabled{opacity:.5;cursor:not-allowed}",
			".vq-card-saved{font-size:12px;color:var(--dsw-alias-state-success-primary,#16a34a);font-weight:650}",
		...
		function Donut(props) {
			...
			return h("div", { className: "vq-donut-wrap" }, h("svg", {
				className: "vq-donut",
				viewBox: "0 0 76 76"
			}, h("circle", {
				cx: 38,
				...
				r,
				fill: "none",
				stroke: "var(--dsw-alias-bg-layer-2,#f2f2f2)",
				strokeWidth: 9
			}), h("circle", {
				cx: 38,
				cy: 38,
				...
				y: 34,
				textAnchor: "middle",
				className: "vq-donut-val"
			}, fmtPct(p)), h("text", {
				x: 38,
				y: 48,
				textAnchor: "middle",
				className: "vq-donut-lbl"
			}, props.label)), h("span", { className: "vq-donut-time" }, resetIn != null ? fmtCountdown(resetIn) + " 后重置" : "重置时间未知"));
		}
		function QuotaFloater() {
			...
			const [open, setOpen] = react.default.useState(() => localStorage.getItem(LS_OPEN) === "1");
			const [ball, setBall] = react.default.useState(() => loadPos(LS_BALL, () => ({
				x: (typeof window !== "undefined" ? window.innerWidth : 1200) - 78,
				y: (typeof window !== "undefined" ? window.innerHeight : 800) - 146
			})));
			const [win, setWin] = react.default.useState(() => ({
				x: (typeof window !== "undefined" ? window.innerWidth : 1200) - 420,
				y: 80
			}));
			const [now, setNow] = react.default.useState(Date.now());
			const dragRef = react.default.useRef(null);
			const load = react.default.useCallback(async () => {
				setLoading(true);
				try {
					const json = await (await fetch(API_URL, {
						method: "POST",
			...
			const closeToBall = () => {
				...
				const GAP = 10;
				const M = 8;
				let nx = ball.x + 32 > vw / 2 ? win.x + PW + GAP : win.x - 64 - GAP;
			...
			const nearestReset = windows.length ? windows.reduce((acc, w) => {
			...
			if (!open) return h("div", {
				...
				onPointerDown: (e) => startDrag(e, "ball"),
				title: "火山方舟额度 · 点击展开（" + (error || (data ? "已用 " + fmtPct(maxUsed) : "加载中")) + "）"
			}, h("span", {
				className: "vq-ball-num",
				style: { color: quotaColor(error ? 100 : maxUsed) }
			}, ballVal), h("span", { className: "vq-ball-lbl" }, ballSub));
			const children = [];
			children.push(h("div", {
				key: "titlebar",
				className: "vq-titlebar",
				onPointerDown: (e) => startDrag(e, "win")
			}, h("div", { className: "vq-titlegroup" }, h("span", { className: "vq-eyebrow" }, "QUOTA MONITOR"), h("span", { className: "vq-title" }, "火山方舟额度")), h("span", { className: "vq-badge" }, data ? data.plan === "agent" ? "Agent Plan" : "Coding Plan" : "…"), h("span", { className: "vq-spacer" }), h("div", { className: "vq-header-actions" }, h("button", {
				key: "refresh",
				className: "vq-ibtn",
				onClick: () => load(),
				title: "立即刷新",
			...
			const body = [];
			if (loading && !data) body.push(h("div", {
				key: "loading",
				className: "vq-loading"
			}, "加载中…"));
			if (error) body.push(h("div", {
				key: "err",
				className: "vq-err"
			}, error + "\n\n提示：AK/SK 请在「设置 → 插件」页的「火山方舟额度」卡片里填写（存于 DSH 凭据库，浏览器不保存密钥）。"));
			if (hot.length > 0) body.push(h("div", {
				key: "warn",
				className: "vq-warn"
			...
			if (data && data.windows.length > 0) {
				body.push(h("div", {
					key: "stats",
					className: "vq-stats"
				}, h("div", { className: "vq-stat" }, h("span", { className: "vq-stat-label" }, "已用（最高）"), h("span", {
					className: "vq-stat-value",
					style: { color: quotaColor(maxUsed) }
				}, fmtPct(maxUsed)), h("span", { className: "vq-stat-sub" }, hottest ? windowLabel(hottest.name) : "")), h("div", { className: "vq-stat" }, h("span", { className: "vq-stat-label" }, "剩余"), h("span", { className: "vq-stat-value" }, fmtPct(hottest ? hottest.remainingPercent : null)), h("span", { className: "vq-stat-sub" }, hottest ? "占" + windowLabel(hottest.name) : "")), h("div", { className: "vq-stat" }, h("span", { className: "vq-stat-label" }, "最近重置"), h("span", {
					className: "vq-stat-value",
					style: { fontSize: 14 }
				}, nearestReset != null && nearestReset !== Infinity ? fmtCountdown(nearestReset - now) : "—"), h("span", { className: "vq-stat-sub" }, "自动刷新 60s"))));
				body.push(h("div", {
					key: "donuts",
					className: "vq-quota"
				}, data.windows.map((w) => h(Donut, {
					key: w.name,
				...
				body.push(h("div", {
					key: "detail",
					className: "vq-panel2"
				}, h("span", { className: "vq-ptitle" }, "窗口明细"), data.windows.map((w) => {
					const color = quotaColor(w.usedPercent);
					const resetIn = w.resetAt ? w.resetAt - now : null;
					return h("div", {
						key: w.name,
						className: "vq-prow"
					}, h("span", { className: "vq-pname" }, windowLabel(w.name)), h("div", { className: "vq-pbar" }, h("div", {
						className: "vq-pbar-fill",
						style: {
							width: Math.max(2, Math.min(100, w.usedPercent)) + "%",
							background: color
						}
					})), h("span", { className: "vq-pcost" }, fmtPct(w.usedPercent) + " · " + (w.used !== null ? fmtNum(w.used) + "/" + (w.quota ? fmtNum(w.quota) : "?") : "") + (resetIn != null ? " · " + fmtCountdown(resetIn) : "")));
				})));
			} else if (data && !error) body.push(h("div", {
				key: "empty",
				className: "vq-loading"
			}, "暂无额度数据（可能未订阅 Coding/Agent Plan）"));
			children.push(h("div", {
				key: "body",
				className: "vq-body"
			}, body));
		...
		function ConfigCard() {
			...
			const [ak, setAk] = react.default.useState("");
			const [sk, setSk] = react.default.useState("");
			const [plan, setPlan] = react.default.useState(() => localStorage.getItem(LS_PLAN) || "auto");
			const [akCfg, setAkCfg] = react.default.useState({
				configured: false,
				writable: true
			});
			const [skCfg, setSkCfg] = react.default.useState({
				configured: false,
				writable: true
			});
			...
			const refreshStatus = react.default.useCallback(async () => {
				try {
					const j = await (await fetch(CFG_URL)).json();
					if (j.ok) {
						setAkCfg({
							configured: !!j.ak.configured,
							source: j.ak.source,
							writable: !!j.ak.writable
						});
						setSkCfg({
							configured: !!j.sk.configured,
							source: j.sk.source,
							writable: !!j.sk.writable
						});
					}
				} catch {}
			}, []);
			...
			const save = async () => {
				...
				setErr("");
				try {
					const j = await (await fetch(CFG_URL, {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({
							ak,
			...
			const clear = async () => {
				...
				setErr("");
				try {
					const j = await (await fetch(CLEAR_URL, { method: "POST" })).json();
			...
			const credentialConfigured = akCfg.configured && skCfg.configured;
			return h("li", { className: "vq-card" }, h("button", {
				type: "button",
				className: "vq-card-header",
				"aria-expanded": open,
				onClick: () => setOpen(!open)
			}, h("span", { className: "vq-card-headtext" }, h("span", { className: "vq-card-name" }, "火山方舟额度"), h("span", { className: "vq-card-desc" }, "Coding Plan / Agent Plan 额度实时查看的访问密钥与套餐类型")), h("span", { className: "vq-card-status" + (credentialConfigured ? "" : " vq-card-status-pending") }, credentialConfigured ? "已配置" : "待配置"), dirty ? h("span", { className: "vq-card-pending" }, "未保存") : null, h("span", { className: "vq-card-chevron" + (open ? " vq-card-chevron-open" : "") }, h(Chevron))), open ? h("div", { className: "vq-card-body" }, h("div", { className: "vq-card-field" }, h("label", {
				className: "vq-card-label",
				htmlFor: "vq-cfg-ak"
			}, "AccessKey ID" + (akCfg.configured ? "（已配置" + (akCfg.source ? " · " + akCfg.source : "") + "）" : "（未配置）")), h("input", {
				id: "vq-cfg-ak",
				className: "vq-card-input",
				value: ak,
				placeholder: "AKLT…",
				disabled: !akCfg.writable,
				onChange: edit(setAk)
			})), h("div", { className: "vq-card-field" }, h("label", {
				className: "vq-card-label",
				htmlFor: "vq-cfg-sk"
			}, "Secret AccessKey" + (skCfg.configured ? "（已配置" + (skCfg.source ? " · " + skCfg.source : "") + "）" : "（未配置）")), h("input", {
				id: "vq-cfg-sk",
				className: "vq-card-input",
				type: "password",
				value: sk,
				placeholder: "留空并保存 = 清除该项",
				disabled: !skCfg.writable,
				onChange: edit(setSk)
			})), h("div", { className: "vq-card-field" }, h("label", {
				className: "vq-card-label",
				htmlFor: "vq-cfg-plan"
				...
				id: "vq-cfg-plan",
				className: "vq-card-select",
				value: plan,
				onChange: edit(setPlan)
			}, h("option", { value: "auto" }, "自动探测（coding → agent）"), h("option", { value: "coding" }, "Coding Plan"), h("option", { value: "agent" }, "Agent Plan"))), h("div", { className: "vq-card-hint" }, "密钥存于 DSH 凭据库（~/.dsh/.credentials.yaml，环境变量优先），浏览器不保存密钥、host 只报状态不返回值。" + (!anyWritable ? " 当前凭据来自只读来源（环境变量），请在环境变量处修改。" : "")), err ? h("div", { className: "vq-err" }, err) : null, h("div", { className: "vq-card-footer" }, saved ? h("span", { className: "vq-card-saved" }, "已保存，悬浮球已刷新") : null, h("span", { className: "vq-spacer" }), anyWritable ? h("button", {
				type: "button",
				className: "vq-card-discard",
				onClick: clear,
				disabled: busy
			}, "清除凭据") : null, h("button", {
				type: "button",
				className: "vq-card-save",
				onClick: save,
				disabled: busy || !dirty
			}, busy ? "保存中…" : "保存"))) : null);
		}
		function apply(ctx) {
			...
			ctx.effect(() => ctx.slots.inject("shell.overlay", () => ctx.slots.register({
				name: "shell.overlay",
				id: "dsh-volcark-quota-overlay",
				order: 60
			}, QuotaFloater)), "dsh-volcark-quota: overlay floater");
			ctx.effect(() => ctx.slots.inject("settings.plugin.item", () => ctx.slots.register({
				name: "settings.plugin.item",
				id: "dsh-volcark-quota",
				order: 90
			}, ConfigCard)), "dsh-volcark-quota: config card");
		}
		//#endregion
		exports.apply = apply;
