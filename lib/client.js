	...
	factory: (require) => {
		...
		*/
		...
		function panelFromBall(b) {
			...
			const GAP = 10;
			const M = 8;
			let x = b.x + 31 > vw / 2 ? b.x - PW - GAP : b.x + 62 + GAP;
		...
		function QuotaFloater() {
			...
			const closeToBall = () => {
				...
				const GAP = 10;
				const M = 8;
				let nx = ball.x + 31 > vw / 2 ? win.x + PW + GAP : win.x - 62 - GAP;
			...
			}, h("span", { className: "vq-title" }, "火山方舟 · 额度"), h("span", { className: "vq-badge" }, data ? data.plan === "agent" ? "Agent Plan" : "Coding Plan" : "…"), h("span", { className: "vq-spacer" }), h("button", {
				key: "refresh",
				className: "vq-ibtn",
				onClick: () => load(),
				title: "立即刷新",
		...
		function ConfigCard() {
			...
			}, h("span", { className: "vq-card-headtext" }, h("span", { className: "vq-card-name" }, "火山方舟额度"), h("span", { className: "vq-card-desc" }, "Coding Plan / Agent Plan 额度实时查看的访问密钥与套餐类型")), dirty ? h("span", { className: "vq-card-pending" }, "未保存") : null, h("span", { className: "vq-card-chevron" + (open ? " vq-card-chevron-open" : "") }, h(Chevron))), open ? h("div", { className: "vq-card-body" }, h("div", { className: "vq-card-field" }, h("label", {
